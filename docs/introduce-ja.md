...
- **AIが作る重複コードの検出**: AIアシスタントが改善時に作りがちな `_enhanced`、`_fixed` といったファイルの検出が容易に
- **構造ベースの比較**: 変数名の違いを超えた本質的な類似度検出

特に、AIアシスタントとの共同作業で生まれる重複コードの検出は、現代的な開発フローにおいて重要な機能になっています。AIは元のファイルをコピーしてから改善版を作る傾向があり、これらが比較的簡単に検知できるようになりました。
