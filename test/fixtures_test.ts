...
function runFixtureTests() {
  ...
  for (let i = 0; i < similarFiles.length; i += 2) {
    ...
    const aptedCustomScore = calculateAPTEDSimilarity(code1, code2, {
      renameCost: 0.3,
    });

    const passed =
      levScore >= SIMILAR_THRESHOLD || aptedScore >= SIMILAR_THRESHOLD || aptedCustomScore >= SIMILAR_THRESHOLD;
    ...
    console.log(
      `  APTED:              ${(aptedScore * 100).toFixed(1)}% ${aptedScore >= SIMILAR_THRESHOLD ? "✓" : "✗"}`,
    );
    console.log(
      `  APTED (rename=0.3): ${(aptedCustomScore * 100).toFixed(1)}% ${aptedCustomScore >= SIMILAR_THRESHOLD ? "✓" : "✗"}`,
    );
  ...
  for (let i = 0; i < dissimilarFiles.length; i += 2) {
    ...
    const passed =
      levScore < DISSIMILAR_THRESHOLD || aptedScore < DISSIMILAR_THRESHOLD || aptedCustomScore < DISSIMILAR_THRESHOLD;
    ...
    console.log(
      `  APTED:              ${(aptedScore * 100).toFixed(1)}% ${aptedScore < DISSIMILAR_THRESHOLD ? "✓" : "✗"}`,
    );
    console.log(
      `  APTED (rename=0.3): ${(aptedCustomScore * 100).toFixed(1)}% ${aptedCustomScore < DISSIMILAR_THRESHOLD ? "✓" : "✗"}`,
    );
    console.log(`  Result: ${passed ? "PASS" : "FAIL"}`);
  }
  ...
  for (let i = 0; i < edgeCaseFiles.length; i += 2) {
    ...
    try {
      ...
      aptedCustomScore = calculateAPTEDSimilarity(code1, code2, {
        renameCost: 0.3,
      });
    } catch (e) {
      error = e;
  ...
  const similarResults = results.filter(
    (r) => r.name.startsWith("similar") || r.file1.startsWith("function_rename") || r.file1.startsWith("class_rename"),
  );
  const dissimilarResults = results.filter((r) => !similarResults.includes(r));

  // For similar code, which algorithm performs best?
  let levBestSimilar = 0,
    aptedBestSimilar = 0,
    aptedCustomBestSimilar = 0;
  ...
  let levBestDissimilar = 0,
    aptedBestDissimilar = 0,
    aptedCustomBestDissimilar = 0;
