...
async function runTSEDBenchmark() {
  ...
  const parsedTests: Array<{ name: string; ast1: any; ast2: any; code1: string; code2: string }> = [];
  ...
  for (const { name, lines } of testSizes) {
    ...
    console.log(`| ${name.padEnd(20)} | ${parseTime.toFixed(2).padStart(10)}ms | ${nodeCount.toString().padStart(9)} |`);
  }
  ...
  for (const test of parsedTests) {
    ...
    console.log(`| ${test.name.padEnd(20)} | ${tsedDefaultTime.toFixed(2).padStart(15)}ms | ${tsedRefactorTime.toFixed(2).padStart(19)}ms | ${levTime.toFixed(2).padStart(14)}ms | ${aptedStringTime.toFixed(2).padStart(15)}ms |`);
  }
  ...
  const operations = [
    { name: 'Rename=1.0, Delete=1.0, Insert=1.0', options: { renameCost: 1.0, deleteCost: 1.0, insertCost: 1.0 } },
    { name: 'Rename=0.3, Delete=1.0, Insert=1.0', options: { renameCost: 0.3, deleteCost: 1.0, insertCost: 1.0 } },
    { name: 'Rename=1.0, Delete=1.0, Insert=0.8', options: { renameCost: 1.0, deleteCost: 1.0, insertCost: 0.8 } },
    { name: 'Rename=0.3, Delete=1.0, Insert=0.8', options: { renameCost: 0.3, deleteCost: 1.0, insertCost: 0.8 } }
  ];
