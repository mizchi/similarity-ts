...
async function quickBenchmark() {
  ...
  for (const { name, size } of sizes) {
    ...
    results.forEach(r => {
      console.log(`  ${r.algorithm}: ${r.result.averageTime.toFixed(2)}ms (±${r.result.standardDeviation.toFixed(2)}ms)`);
    });
  }
  ...
  try {
    ...
    const realResults = await benchmark.benchmarkPairwiseComparison(
      userServiceCode,
      productServiceCode
    );
