...
"#;
    ...
    assert!(
        ...
        "Expected similarity between 30-70%, got {:.2}%",
        similarity * 100.0
    );
}
...
"#;
    ...
    println!(
        "Enhanced similarity between bubble_sort and binary_search: {:.2}%",
        similarity * 100.0
    );
...
"#;
    ...
    println!(
        "Enhanced similarity between process and validate methods: {:.2}%",
        similarity * 100.0
    );
...
fn extract_function_body(
    code: &str,
    func: &similarity_core::language_parser::GenericFunctionDef,
) -> String {
