...
fn test_find_duplicates_in_python_file() {
    ...
    let process_funcs: Vec<_> = functions.iter()
        .filter(|f| f.name.contains("process") || f.name.contains("transform"))
        .collect();
    
    assert!(process_funcs.len() >= 4, "Expected at least 4 process/transform functions, found {}", process_funcs.len());
    
    // Compare the standalone functions
    let process_data = functions.iter().find(|f| f.name == "process_data" && !f.is_method).unwrap();
    let transform_data = functions.iter().find(|f| f.name == "transform_data" && !f.is_method).unwrap();
