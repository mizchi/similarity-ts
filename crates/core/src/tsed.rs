...
pub fn calculate_tsed_from_code(code1: &str, code2: &str, filename1: &str, filename2: &str, options: &TSEDOptions) -> Result<f64, String> {
...
mod tests {
    ...
    fn test_identical_code() {
        ...
        let similarity = calculate_tsed_from_code(code, code, "test1.ts", "test2.ts", &options).unwrap();
    ...
    fn test_renamed_function() {
        ...
        let similarity = calculate_tsed_from_code(code1, code2, "test1.ts", "test2.ts", &options).unwrap();
