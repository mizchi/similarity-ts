...
pub fn calculate_enhanced_similarity(
    ...
    let structural_similarity = if max_size > 0.0 {
        1.0 - (distance / max_size)
    } else {
        1.0
    };
    ...
    let total_weight = options.structural_weight + options.size_weight + options.type_distribution_weight;
    let combined_similarity = (
        structural_similarity * options.structural_weight +
...
fn calculate_complexity_recursive(node: &TreeNode, depth: usize) -> usize {
    ...
    match node.label.as_str() {
        "if_expression" | "if_statement" | "loop_expression" | 
...
mod tests {
    ...
    fn test_size_similarity() {
        ...
        assert!(similarity < 0.5, "Expected low similarity due to size difference, got {}", similarity);
    }
}