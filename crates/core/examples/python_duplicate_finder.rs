...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    for i in 0..functions.len() {
        for j in i + 1..functions.len() {
            ...
            let func1_body = lines
                [(func1.body_start_line as usize - 1)..(func1.body_end_line as usize)]
                .join("\n");
            let func2_body = lines
                [(func2.body_start_line as usize - 1)..(func2.body_end_line as usize)]
                .join("\n");
    ...
    if similar_pairs.is_empty() {
        ...
        let total_lines: u32 =
            similar_pairs.iter().map(|(f1, _, _)| f1.end_line - f1.start_line + 1).sum();
