...
test('TSED: formula verification with metrics', () => {
  ...
  assert.equal(metrics.maxNodes, Math.max(metrics.tree1Nodes, metrics.tree2Nodes), 
...
test('TSED: operation costs affect similarity', () => {
  ...
  assert(lowInsertCost > highInsertCost, 
...
test('TSED: parameter optimization from paper (section 5.3)', () => {
  ...
  assert(tsedOptimal >= tsedDefault, 
...
test('TSED: value bounds validation', () => {
  ...
  for (let i = 0; i < codes.length; i++) {
    for (let j = 0; j < codes.length; j++) {
      ...
      assert(tsed >= 0 && tsed <= 1, 
        `TSED should be between 0 and 1, got ${tsed} for codes[${i}] vs codes[${j}]`);
